...
export function TransversalResultSVG({ ...props }: ITransversalResultsDisplay) {
  ...
  useEffect(() => {
    ...
    if (
      state.results.anglesResistanceDiagramPoints != null &&
    ...
    if (
      ...
      for (let angle of sortedAngles.sort(function (a, b) {
        return a - b;
      })) {
        if (angle > graphAngle) {
      ...
      if (Math.abs(mx) > 0.005 || Math.abs(my) > 0.005) {
        //FIND SAFETY FACTOR
        let [mr, nr] = fitLinearPolynomialFunction(
          beforeMx,
          afterMx,
          beforeMy,
          afterMy
        ...
        const intersectDistance = Math.sqrt(
          Math.pow(xIntersect, 2) + Math.pow(yIntersect, 2)
        );
      ...
      const absMaxMx =
        Math.abs(maxMx) > Math.abs(minMx) ? Math.abs(maxMx) : Math.abs(minMx);
      const absMaxMy =
        Math.abs(maxMy) > Math.abs(minMy) ? Math.abs(maxMy) : Math.abs(minMy);
      ...
      svg
        ...
        .style("font-size", "0.8rem")
        .text(
          `FS:${safetyFactor.toFixed(2)} - Mx:${mx.toFixed(
            2
          )} kN.m - My:${my.toFixed(2)} kN.m`
        )
        .style("fill", `${safetyFactor > 1 ? "green" : "red"}`)
  ...
  return (
    <>
      <div
        className="flex border-brandPurple-300 border-2 rounded-2xl bg-white w-full justify-center text-center items-center aspect-square md:aspect-video mt-2"
        id="svg-d3-results"
      >
