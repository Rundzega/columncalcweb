...
const initialState: IInputState = {
  ...
  diagramPoints: 90,
  loadIncrements: 5,
  maxIterationsPerIncrement: 2000,
  ...
  switch (action.type) {
    case "field": {
      return {
        ...
        results: { ...state.results, isResultsAvailable: false },
      };
    }

    case "add-rectangle": {
      const newRectangleList = [
        ...state.rectangleList,
        {
    ...
    case "remove-rectangle": {
      ...
      const rectLeftVertices =
        deletedRectangle.xCenterCoordinate - deletedRectangle.width / 2;
      const rectRightVertices =
        deletedRectangle.xCenterCoordinate + deletedRectangle.width / 2;
      const rectTopVertices =
        deletedRectangle.yCenterCoordinate + deletedRectangle.height / 2;
      const rectBottomVertices =
        deletedRectangle.yCenterCoordinate - deletedRectangle.height / 2;
      ...
      rebarListCopy.forEach((rebar) => {
        ...
        const rebarBottomExtreme =
          rebar.yCenterCoordinate - rebar.diameter / 20;

        if (
          rectLeftVertices <= rebarLeftExtreme &&
      ...
      return {
        ...state,
        rebarList: rebarListAfterDelete,
        rectangleList: rectanglesDeletedRow,
        selectedElement: { index: null, type: null },
        results: { ...state.results, isResultsAvailable: false },
      };
    }
    ...
    case "update-rect-rebar-list": {
      return {
        ...state,
        rectangleList: action.payload.rectangleList,
        rebarList: action.payload.rebarList,
      };
    }
    ...
    case "select-element": {
      ...
      newSelectedElementIndex =
        rectId.length > rebarId.length ? rectId[0].index : rebarId[0].index;
      newSelectedElementType =
        rectId.length > rebarId.length ? "rectangle" : "rebar";
      ...
      if (state.selectedElement.index === newSelectedElementIndex) {
        ...
        newRectList.forEach((rectangle) => {
          rectangle.isHighlighted =
            rectangle.index === newSelectedElementIndex ? true : false;
        });
        newRebarList.forEach((rebar) => {
          rebar.isHighlighted =
            rebar.index === newSelectedElementIndex ? true : false;
        });
      }

      return {
        ...state,
        rectangleList: newRectList,
        rebarList: newRebarList,
        selectedElement: {
          index: newSelectedElementIndex,
          type: newSelectedElementType,
        },
      };
    }
    ...
    case "display-longitudinal-results": {
      return {
        ...state,
        longitudinalDisplayResults: action.payload,
        currentResultType: "longitudinal",
      };
    }
    ...
    case "display-transversal-results": {
      return {
        ...state,
        transversalDisplayResults: action.payload,
        currentResultType: "transversal",
      };
    }
